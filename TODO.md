...
- [x] **Gemini writes through a paste loop, not an API.** `scripts/brief.ts` emits `out/brief-<id>.md` alongside the JSON; `prompts/WRITER_SKILL.md` carries the seven measured style rules and the accent contract and is pasted once. `npm run draft <id> <file>` ingests the reply, tolerates a chat box's fences and prose, runs verifyDraft, and refuses to write anything when a rule broke. A verified draft feeds `scripts/tts.ts <id> <draft.json>` unchanged.
- [ ] **Project screen in the UI.** Accept in Discover currently ends the flow: it writes the ledger and stops. The stage bar (Facts → Chart → Brief → Draft → Voice → Render) should run each existing script as an SSE job — the job registry from phase 1 was built for this — with the Brief step offering a copy button and the Draft step a paste box.
